...
class FavoriteSeries
{
	public:
		...
		void sorting(vector<myEpisode> &list) const;
		void sortActors(vector<actor> & act) const;
		int slotint(string slot) const;
		void alreadyAdded(vector<actor> &act, actor a) const;
	private:
		string userName;
		string userSurname;
